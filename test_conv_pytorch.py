...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1,3,(4,4), stride=2)
        self.barch_norm1 = nn.BatchNorm2d(3)
        self.conv2 = nn.Conv2d(3,5,(3,3),stride=2)
        self.barch_norm2 = nn.BatchNorm2d(5)
        self.fc1 = nn.Linear(6*6*5,30)
        self.fc2 = nn.Linear(30,10)
    def forward(self, x):
        x = self.conv1(x)
        x = F.leaky_relu(self.barch_norm1(x),0.1)
        x = self.conv2(x)
        x = F.leaky_relu(self.barch_norm2(x),0.1)
        x = x.view(-1,6*6*5)
...
test_imgs = torch.from_numpy(test_imgs).view(10000,1,28,28).float()
train_imgs = torch.from_numpy(train_imgs).view(60000,1,28,28).float()
